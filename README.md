...
- Поддержка 2 аккаунтов на устройстве (быстрое переключение в `/settings`)
...
- На `/chats` в блоке поиска ввести ник (минимум 2 символа), найти пользователя и создать с ним DM
- В `/settings` проверить:
  - редактирование профиля (ник, аватар, bio)
  - приватность (видимость, read receipts, typing, discoverability)
  - уведомления (push/email/marketing)
  - безопасность (смена пароля, выход со всех устройств)
...
## Web Push (VAPID) настройка
1. Сгенерируйте VAPID-ключи (например, через `web-push generate-vapid-keys` на машине с доступом к npm).
2. Заполните в `.env`:
   - `VAPID_PUBLIC_KEY`
   - `VAPID_PRIVATE_KEY`
   - `VAPID_SUBJECT` (например, `mailto:admin@example.com`)
3. Перезапустите `pnpm dev`.
4. В приложении откройте `/settings` -> `Уведомления` -> `Запросить push-разрешение`.

## Next steps
1. Добавить E2EE (Signal/MLS)
2. Серверная доставка payload в web-push endpoint (HTTP Web Push send pipeline)
